...
- `PASS` requires a zero exit code and a result file with at least one test run and no failure, error, skip or expected failure. A timeout, a crash, an accidental `os._exit` or `SystemExit` at import, a syntax error or an empty file is `FAIL`. A submission that tampers with the verdict can still forge a `PASS` (see the safety boundary).
...
**A `PASS` trusts the submission not to tamper.** The submission is imported into the same process that runs the hidden suite and writes the result file. It can write that file itself and exit, or patch `unittest`, and get `PASS`. `progress record` then stores that challenge as completed. This was reproduced on 2026-09-26: a three-line `mod.py` with no `paginate` got `PASS`, "7 tests passed". `tests/test_challenges.py` pins this behaviour to these docs. `FAIL` is guaranteed only for accidental exits, crashes and timeouts. Hiding the result path would not help: it is always `../result.json` from the child's working directory, and in-process code can patch the runner anyway. Only the V-04 isolation boundary can close this.
