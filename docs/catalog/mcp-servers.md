...
| 21st Magic MCP (`21st`) | execution | http (hosted, `https://21st.dev/api/mcp`) | API_KEY_21ST from 21st.dev/mcp; Builder plan free (5 Design Bug Bot reviews, unlimited code and prompt copies, full marketplace, no card); `generate`/`iterate_generation` only on Builder + AI ($15/month billed yearly, 500–2,000 credits/month); ISC | yes, every call | `{"mcpServers": {"21st": {"url": "https://21st.dev/api/mcp", "headers": {"x-api-key": "YOUR_21ST_API_KEY"}}}}` or `claude plugin marketplace add 21st-dev/magic-mcp` then `/plugin install 21st` | https://github.com/21st-dev/magic-mcp |
...
Design stack: the 21st row is the component supply of the frontend pipeline described in [community-skills.md](community-skills.md#design-stack-frontend); it runs after `impeccable` has written `DESIGN.md` and before the ponytail review (`magic-mcp guided-by impeccable` in `docs/skills-graph/`). Tools: `search`, `get_component`, `get_inspiration`, `search_logo`, plus `generate` and `iterate_generation` when `get_usage.aiGenerationEnabled` is true. The key is a credential: the owner creates it and puts it in the config; the agent never types it.
