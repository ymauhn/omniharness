...
Outside `dataDir`: `.omniharness/runtime/laya-multilingual/<rev>/` is the opt-in local Laya checkpoint; usage reconciliation *reads* (never writes) `~/.claude/projects/**/<session>.jsonl` and `~/.codex/sessions/**/rollout-*.jsonl`, the CLIs' own transcript files.
...
6. **Extension sandbox.** Generated code runs in a separate low-privilege Node process with no filesystem/network/`require` access and code generation disabled in the `vm` context, so a malicious or crashing generated tool fails closed without touching the app.
