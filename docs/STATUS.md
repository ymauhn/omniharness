...
- **POSIX only: a merge test command that leaves a background child holding its output waits for that child.** `review.mjs`'s `runTestCommand` resolves on the pipes' `close`, so on Linux/macOS a descendant that keeps stdout open holds the merge until it exits or the 10-minute timeout kills the tree. Out of scope for the Windows-first V1; not fixed.
