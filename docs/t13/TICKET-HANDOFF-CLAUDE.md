...
Latest implementation: **NATIVE CLAUDE/CODEX OFFLINE ADAPTERS VALIDATED — live agent-tree containment/accounting remains open**, starting from clean `61e00eae33f9b0390987ecf9206a67154432a659`. The process, usage, cancellation and recovery seams pass offline adversarial tests; the full battery passed 140/140 Python tests without skips. The aggregate command still exits 1 on two explicitly owner-preserved Claude installation-policy differences; the graph passed separately. Both native login-status preflights succeeded, but no authenticated model turn or current Docker containment acceptance ran. Read the newest journal and validation before continuing or committing.
...
| T13 S3 swarm | Offline driver, ledger, whole-tree Claude parser, worktree audits and offline-tested native Claude/Codex process adapters implemented; historical container proof retained | Current Docker daemon probe failed; live all-role executor/usage/isolation wiring and E1 controls remain; ordinary native allowance work is authorised |
...
Historical September 21 update: Docker recovery and the offline container process probe then passed 12 checks; reuse `harness/sandbox_probe.py` and its recorded evidence only after rechecking the current host. On September 25 Docker Desktop again failed to start its Linux daemon after nondestructive socket-only backups, so current all-role containment is **not** accepted. The next implementation is the coordinator/native agent tool-execution bridge with attributable usage after a fresh successful host probe, not a blanket model-charge approval round. A passing fixed probe does not certify an agent connected to that profile.
