...
On this Windows host, `taskkill /T` from the Codex command sandbox returned access denied; the same disposable parent/child test stopped both processes outside that sandbox. The process runner conservatively records `unknown` when tree termination cannot be confirmed. Docker Desktop's engine was unavailable during the latest probe, so the historical 12/12 offline container result below is **not** a new host acceptance for this checkpoint. A nondestructive retry backed up only socket-only runtime directories, but Docker 4.85.0 recreated a bad `dockerInference` listener and failed again. The September 25 validation and handoff journal record the exact attempts and preserved socket backups.
