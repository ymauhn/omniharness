...
- Added 2026-09-14, second round: RuFlo read and compared (`docs/experiments/ruflo-comparison-2026-09-14.md`; verdict: not installed, catalogued, patterns mined) and the master ticket **T13, OmniHarness v3 (Swarm & Autonomy Era)**: a session envelope with three modes (Swarm, Balanced, Strict) signed at `/omniharness` start, GOAP as data (`needs:`, `cost:`, `alternatives`, reroute per mode), a swarm driver with worktrees, a scope map, an integrator and a reviewer, a cross-run seen store, deterministic hooks, benchmarks B8 and B9. Sub-tickets S1–S7 in `docs/commercial/TICKETS.md`. The owner answered T13's pending items on 2026-09-14: invariant 2 amendment approved (the envelope signed at the start is the explicit prior yes inside its scope and budget), default mode Balanced, default budget US$ 2.00 (about 750 k tokens) with the warning at 80 %, B8 and B9 approved.
