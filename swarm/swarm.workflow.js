...
const completeUsage = u => u && units.every(k => finite(u[k])) && Number.isInteger(u.tokens)
...
if (!env || !['balanced', 'swarm'].includes(env.mode) || !completeUsage(env.remaining) || !completeUsage(a.perAgent) || !units.every(k => a.perAgent[k] > 0) || !budget || !['reserve', 'settle', 'cancel'].every(k => typeof budget[k] === 'function')) return out
...
try {
  const proof = await isolation.preflight({baseCommit: a.baseCommit, tasks: a.tasks})
  if (!proof || proof.os_sandbox_verified !== true) { out.parouPor = 'isolation'; return out }
} catch (e) { out.parouPor = 'isolation'; return out }
const remaining = {...env.remaining}
let accountingOK = true, base = a.baseCommit
...
async function invoke(label, prompt, options, lease) {
  let report = null
  try { report = await agent(prompt, {label, ...options, reservation: lease}) } catch (e) { out.warnings.push(label + ': agent failed') }
  let usage = null
  try { usage = await budget.settle(lease) } catch (e) { /* retain the reservation and stop */ }
  if (!completeUsage(usage)) {
    accountingOK = false
  ...
  out.attempts.push({label, report, usage})
  return report
}
...
for (let index = 0; index < layers.length; index++) {
  ...
  for (let i = 0; i < replies.length; i++) {
    try {
      const proof = await isolation.verify(replies[i], {baseCommit: base, scope: jobs[i].task.scope})
      if (!proof || proof.scope_ok !== true || proof.os_sandbox_verified !== true) { out.parouPor = 'isolation'; return out }
    } catch (e) { out.parouPor = 'isolation'; return out }
  }
  const winners = []
...
out.parouPor = !accountingOK ? 'accounting' : out.review && out.review.execution_valid === true && out.review.approved === true && Array.isArray(out.review.findings) && out.review.findings.length === 0 && Array.isArray(out.review.checks) && ['ponytail-review', 'code-review'].every(s => out.review.checks.includes(s)) ? 'complete' : 'review-failed'
out.commit = base
return out
