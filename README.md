...
- **Evidence bundle.** The task's evidence file keeps its last 20 merge attempts (accepted or refused) and its Gauntlet runs: the test command, its exit code and output hash, the diff stat, the token usage the CLI itself recorded, and the reviewer's note.
...
- **Arsenal.** A separate per-project panel, next to Agentes: import/derive versioned agent profiles, review their rules explicitly before activation, and pin a version to a task as `runnable:false` metadata (it never starts a model).
