...
- **Review, diff and gated merge.** A task's diff against its base commit, file by file; "Aprovar e fazer merge" refuses on merge conflicts, an untested change, a failing test command or a repository that moved under it, and otherwise fast-forwards the change into the project's real branch.
