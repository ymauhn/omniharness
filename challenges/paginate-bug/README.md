...
`verify` runs the hidden suite in `hidden/` in a fresh temporary directory and prints a JSON verdict with the submission's and the verifier's sha256. A timeout, a crash or an early exit is `FAIL`, never `PASS`. `progress record` verifies again and appends the verdict to your progress file; `progress show` lists what you completed, also after a restart.
...
**Not a sandbox.** The verifier runs the submission as you, with your files and network. Verify only code you wrote or have read, until the isolation boundary tracked as V-04 in `docs/omniforge/VALIDATION-PENDING.md` holds.
