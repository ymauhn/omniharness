...
export async function startDemo({ repoRoot = REPO_ROOT, tempRoot = process.platform === 'win32' ? process.env.TEMP || os.tmpdir() : os.tmpdir(), createServer = createOmniForgeServer } = {}) {
  ...
  let app;
  try {
    app = createServer({ dataDir, repoRoot: repo });
