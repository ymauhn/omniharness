...
export function createFleet({ openSession, onChange = () => {} }) {
  ...
  // "Sugerir host e skill" marks the suggested host's run button; it never runs anything.
  function taskControls(item, task) {
    const row = one(item, 'div', 'task-run'), run = latestRun(task.id), entry = suggestions.get(task.id) ?? {};
