...
function environment() {
  ...
  const arsenal = { sync() {}, load() {} };
  const extensionsPanel = { sync() {}, load() {} };
  const usagePanel = { load() {} };
  const memoryPanel = { load: () => calls.push('memory-panel') };
  const reviewPanel = { sync() {} };
  const nav = createNavigation({ workspace, tasks, graphs, assets, catalog, copilot, workflows, arsenal, extensionsPanel, usagePanel, memoryPanel, reviewPanel });
...
test('navigation opens saved workflows and refreshes scoped memory controls', () => {
  ...
  nav.showView('unknown'); assert.equal(local.view, 'graphs');
});
