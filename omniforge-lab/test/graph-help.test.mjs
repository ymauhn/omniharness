...
// Exercise the real graph code (graphs.mjs) against a minimal DOM event contract.
function dom() {
  const ids = new Map(), doc = {};
  class Element {
    constructor(tag) { this.tag = tag; this.children = []; this.parentElement = null; this.events = {}; this.dataset = {}; this.style = {}; this.attributes = {}; this._text = ''; this.hidden = false; this.className = ''; this.value = ''; }
    ...
    querySelectorAll(selector) {
      const matches = this.children.flatMap(child => [...(selector === '.graph-node' && child.className === 'graph-node' || selector === child.tag ? [child] : []), ...child.querySelectorAll(selector)]);
      matches.find = undefined; return matches;
    }
  ...
  doc.querySelector = $;
  const addId = (id, parent = doc.body) => { const element = $(`#${id}`); element.id = id; parent.append(element); return element; };
  addId('graph-canvas');
...
function graphPage(localState, api) {
  const page = dom(), opened = [];
  Object.assign(local, localState);
  globalThis.document = page.doc;
  globalThis.window = { innerWidth: 800, innerHeight: 420, addEventListener() {}, open() {} };
  const calls = [];
  const realFetch = globalThis.fetch;
  globalThis.fetch = async (path, options) => { calls.push(path); return { ok: true, status: 200, json: async () => api(path, options) }; };
  bindRefresher(async () => {});
  const catalog = { search: ring => opened.push(['search', page.$('#skill-search').value, ring]) };
  const showView = view => opened.push(['view', view]);
  const memoryPanel = { load() {}, sync() {} };
  const graphs = createGraphs({ assignPane() {}, renderWorkspace() {}, catalog, memoryPanel, showView });
  const buttons = () => page.$('#graph-canvas').querySelectorAll('.graph-node');
  return { ...page, graphs, calls, opened, buttons, byLabel: label => [...buttons()].find(button => button.textContent === label), help: page.$('#graph-help'), restoreFetch: () => { globalThis.fetch = realFetch; } };
}
const row = (ring, name, extra = {}) => ({
  skill_id: `${ring}:${name}`, name, ring, hosts: [], availability: ring === 'installed' ? 'installed' : 'not_installed',
...
const serve = current => (path) => {
  if (current() instanceof Error) throw current();
  const { snapshot_id, rows } = current(), ring = new URL(path, 'http://local').searchParams.get('ring');
...
test('capability graph comes from the catalog snapshot, labels ring and hosts, and never shows stale curation', async () => {
  ...
  const localState = { view: 'graphs', graph: 'capability', graphNode: null, graphModels: {}, projectId: 'a', capability: null, capabilityRequest: 0,
  ...
  const { doc, $, graphs, calls, opened, buttons, byLabel, help, restoreFetch } = graphPage(localState, serve(() => current));
  try {
    graphs.renderGraph();
    assert.match($('#graph-intro').textContent, /Consultando o índice de skills/);
    await graphs.loadCapabilities();
...
test('task and knowledge graphs keep project scope, keyboard focus and help across refreshes', () => {
  ...
  const { doc, $, graphs, byLabel, help, restoreFetch } = graphPage(localState, async () => ({}));
  try {
    graphs.renderGraph();
    ...
    local.graph = 'knowledge'; graphs.renderGraph();
    ...
    assert.match($('#graph-help-docs').textContent, /Projeto A/);
    local.projectId = 'b'; graphs.renderGraph();
...
test('task and knowledge graphs show the most recent items, say what is hidden, and state that session notes are excluded', () => {
  ...
  const localState = { view: 'graphs', graph: 'task', graphNode: null, graphModels: {}, projectId: 'a', notes,
    state: { projects: [{ id: 'a', name: 'Projeto A', root: 'A' }], sessions, tasks } };
  const { $, graphs, byLabel, restoreFetch } = graphPage(localState, async () => ({}));
  try {
    graphs.renderGraph();
