...
test('local API requires a token and preserves project/task/memory boundaries', async t => {
  ...
  const mainModule = await (await fetch(`${base}/app/main.mjs`)).text();
  assert.match(mainModule, /from '\.\/workspace\.mjs'/);
  const parsed = spawnSync(process.execPath, ['--check', '--input-type=module'], { input: mainModule, encoding: 'utf8' });
  assert.equal(parsed.status, 0, parsed.stderr);
  const stateModule = await (await fetch(`${base}/app/state.mjs`)).text();
  assert.match(stateModule, /from '\.\.\/pane-scope\.mjs'/);
  const auth = { 'x-omniforge-token': 'test-token' };
  const authed = path => fetch(`${base}${path}`, { headers: auth });
  assert.equal((await authed('/app/sub/state.mjs')).status, 404, 'the app/ route never resolves a nested path');
  assert.equal((await authed('/app/State.mjs')).status, 404, 'the app/ route is case-sensitive and has no traversal characters');
