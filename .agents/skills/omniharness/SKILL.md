...
4. Report in at most five lines: rules loaded; install state; gate state (`~/.claude/settings.json` carries the ask list and the guard hook; both apply to new sessions only); regressions; the routing rows that matter for the repository the session is in (tests present? a `.tex` and result CSVs? a `.claude/gauntlet-loop.json`?).
